function params_out = Optimize_Theta_BCD_3(params)
% 输入: 参数结构体params
% 输出: 新参数结构体params_out(更新了Theta), 这里在欧几里得梯度的计算上和版本2有差别, 在二次项的求导结果上对了一个系数2
        ...
        for r = 1:R
            for g = 1:G
                ...
                while error_Theta_rg > 1e-3 && iter_Theta_rg <= 20
                    ...
                    % 计算欧几里得梯度
                    Euclid_grad_rg = Calculate_Euclid_grad_rg_3(params.Theta, X, Y, Z, R, G, r, g, M_bar);
