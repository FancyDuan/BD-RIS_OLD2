...
%% 设置发射功率 P_T
Pmax_range_dBm = 10:5:40;                                   % 定义功率范围: 0dBm-40dBm, 步长5dBm
K_range = [8,16];                                           % 定义用户数范围
P_Num = length(Pmax_range_dBm);
K_Num = length(K_range);
SumRate_Record = zeros(P_Num, K_Num);                       % 预分配数组存储结果
...
for num_k = 1 : K_Num
    for num_p = 1 : P_Num
        ...
        while error > 1e-3 && iter <= 100
            ...
            params = Optimize_Theta_BCD_3(params); % 修改了欧几里得梯度计算公式
        ...
        % 记录当前 K, Pmax 下的和速率
        SumRate_Record(num_p, num_k) = Calculate_SINR_SumRate(params).SumRate;
...
for k_idx = 1:length(K_range)
    plot(Pmax_range_dBm, SumRate_Record(:, k_idx), ...
        'Color', colors(k_idx, :), ...
        'Marker', markers(k_idx), ...
        'LineWidth', 1.5, ...
        'MarkerSize', 4);
end
...
title('Sum Rate vs. \(P_{\max}\) under Different Numbers of UEs', 'FontSize', 12);

% 5. 生成图例
legend_labels = arrayfun(@(k) sprintf('K = %d', k), K_range, 'UniformOutput', false);
